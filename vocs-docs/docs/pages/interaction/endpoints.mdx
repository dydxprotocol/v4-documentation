...
You'll need an endpoint to setup the Node client. Grab an RPC/gRPC endpoint from [here](#testnet).
...
config = TESTNET.node # [!code focus]
# Or configuring your endpoints:
# ```python
# config = make_testnet(
...
// Using a pre-configured endpoint. // [!code focus]
const config = Network.testnet().validatorConfig; // [!code focus]
// You can modify the endpoint doing `config.restEndpoint = "...";`
// Connect to the network. // [!code focus]
const client = await ValidatorClient.connect(config); // [!code focus]
```
...
The Indexer client is further divided into two sub-clients associated with the two different protocols. 
You'll need two endpoints to setup each Indexer sub-client. Grab these from [here](#testnet-1).
...
indexer = IndexerClient(network.rest_indexer) # [!code focus]
...
// Using a pre-configured endpoint. // [!code focus]
const config = Network.testnet().indexerConfig; // [!code focus]
...
const client = new IndexerClient(config, apiTimeout); // [!code focus]
...
:::tip[Alternatives]
Moving assets is not restricted to using the Noble client directly. Please see the [Deposits and Withdrawals](/interaction/deposits-and-withdrawals) page.
:::
