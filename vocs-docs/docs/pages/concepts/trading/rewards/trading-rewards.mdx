...
For each successful trade, `Traders` will be rewarded in the `NATIVE_TOKEN` dYdX based on the formula outlined in the below section. Trading rewards are distributed automatically and directly to the trader’s account per block.
...
```
trading reward from taker fee = taker volume X (taker fee rate - max maker rebate - max possible affiliate taker fee share) X C X (1 - protocol revenue share rate)
```
...
```
trading reward from maker fee = maker volume (positive maker fees) X C X (1 - protocol revenue share rate)
```
