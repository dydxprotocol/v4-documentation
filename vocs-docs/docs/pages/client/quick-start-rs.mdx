...
From the same mnemonic/`Wallet` several (independent, non-publicly linked) accounts can be used. Here, we derive the account at index `0`:
...
Entering, managing, and closing positions is made by placing orders. There are two types of orders: short-term orders and long-term orders. # TODO explain difference: https://docs.dydx.exchange/api_integration-trading/short_term_vs_stateful
...
:::note
Please note that the statement is not completed, as we will continue to build a chain of calls from the same builder below.
:::
...
```rust [main.rs]
let (_id, order) = OrderBuilder::new(market, subaccount)
    .limit(OrderSide::Buy, price.to_owned(), BigDecimal::from_str("0.02")?) // [!code focus]
```
...
```rust [main.rs]
    ...
    .reduce_only(false) // [!code focus]
```

```rust [main.rs]
    .reduce_only(false)
    .time_in_force(TimeInForce::Unspecified) // [!code focus]
```

```rust [main.rs]
    .time_in_force(TimeInForce::Unspecified)
    .until(current_block_height.ahead(10)) // [!code focus]
```

```rust [main.rs]
    .until(current_block_height.ahead(10))
    .build(123456)?; // [!code focus]
```

Creating an order requires parameters (`OrderMarketParams`) associated with the traded perpetual.
These can be fetched using the `IndexerClient`. For example for the `ETH-USD` pair:
...
To ease order creation, an `OrderBuilder` is provided to build orders.
...
For more building options please refer to [`OrderBuilder`](https://github.com/dydxprotocol/v4-clients/blob/3e8c7e1b960291b7ef273962d374d9934a5c4d33/v4-client-rs/client/src/node/order.rs#L171) and the [examples](https://github.com/dydxprotocol/v4-clients/tree/3e8c7e1b960291b7ef273962d374d9934a5c4d33/v4-client-rs/client/examples). # Though maybe we can explain them here (the OrderBuilder setters, in another section).

Here, we create a `Buy` order for 3 units of `ETH-USD` at a price of 1500 USDC, with a validity of 10 blocks (~10 seconds):

```rust [main.rs]
let client_id = 123456;
let (order_id, order) = OrderBuilder::new(market, subaccount)
    .limit(OrderSide::Buy, 1500, 3)
    .until(current_block_height.ahead(10))
    .build(client_id)?;
```
...
To broadcast the order to the network, `NodeClient::place_order()` is employed. The account owning the subaccount of the order, and order are required:
...
To cancel an order, the order ID returned by the `OrderBuilder` and the validity of the cancel request must be provided:
