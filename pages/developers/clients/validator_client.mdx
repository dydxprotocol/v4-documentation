...
- Subaccount ID (note for the vast majority of usecases, this will be set to your account address and use the 0th subaccount ID).
- Client ID.
...
Assuming the current block height is 10, the below sample code would place a Short-Term order that is valid if it's placed on the book and matched
by block 11, else it was not included in block 11 and will expire.
