...
- On a cold-start, a full-node would still have all the stateful orders and would send them to the indexer. For short-term orders, the full-node would not know them, nor would the indexer. Since short-term orders only are valid for 40 blocks, within 40 blocks the indexer would have an accurate view of the order book, but for the first 40 blocks it would not.
