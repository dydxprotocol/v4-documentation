...
- **Order flags**: An integer identifying if the order is short-term (`0`), long-term (`64`), conditional (`32`), or TWAP (`128`);
...
- **Good until Block**: Validity of the order. It is an integer value, different for short-term and long-term orders:
  - **Short-term**: Short term orders have a maximum validity of current block height + `ShortBlockWindow`. Currently, `ShortBlockWindow` is 40 blocks, or about 30 seconds;
