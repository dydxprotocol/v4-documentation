...
### Setup a Setup API Keys
On the dYdX.trade frontend click `More → API Trading Keys`.
Click `Generate New API Key`.
The app generates a new keypair:
API Wallet Address
Private Key (one-time view)
Save your Private Key immediately. It will not be shown again and is not stored by dYdX.
...
const wallet = await LocalWallet.fromPrivateKey(DYDX_TEST_PRIVATE_KEY, BECH32_PREFIX);
